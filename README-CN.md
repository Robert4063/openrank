...
## 项目分工
- **杨家辉** — React前端框架的构建，GitHub中issue,comments开源数据爬取与清洗和维度导入，OpenDigger数据爬取，后端技术栈、后端数据库的连接和架构，前后端响应系统构建，构想四维评估体系，构建计算PHAM v2.0模型，贡献者分析维度决策。
- **庞毅飞** — MaxKB智能体的本地部署、网络连接与（知识库、模型）构建，GitHub中star,fork开源数据爬取与清洗、整理，PHAM v2.0模型构建的可行性分析，市场需求调研与相关数据分析，PPT制作与报告撰写。
